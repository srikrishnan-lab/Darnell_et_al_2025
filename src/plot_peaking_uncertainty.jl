...
function plot_peaking_ensemble(q_thresh, parameters, emissions, slr_out, temp_out, ais_out)
    ...
    for (j, idx) in pairs(low_all_idx)
        ...
        if !isnothing(exceed_ais_idx)
            ...
            temp_exceed_max[j] = temp_out[idx, exceed_ais_idx]
        end
    end
    ...
    plot_quantiles(emissions, parameters, unique(parameters[!, :t_peak]), low_all_idx, "CO₂ Emissions\n(GtCO2/yr)", pk_colors, f=gts[1, 1])
    plot_quantiles(slr_out, parameters, unique(parameters[!, :t_peak]), low_all_idx, "Global Mean\nSea Level (m)", pk_colors, f=gts[1, 2], limits=(0, 1.5))
    ...
    plot_quantiles(ais_out, parameters, unique(parameters[!, :t_peak]), low_all_idx, "AIS Contribution (m)", pk_colors, f=gts[1, 3], limits=(-0.05, 0.75))
...
f_base = plot_peaking_ensemble(0.1, parameters, emissions, slr_out, temp_out, ais_out)
save("figures/peaking_threshold.png", f_base)

# make supplemental figure, top 20%
f_base = plot_peaking_ensemble(0.2, parameters, emissions, slr_out, temp_out, ais_out)
save("figures/peaking_threshold_supp.png", f_base)
