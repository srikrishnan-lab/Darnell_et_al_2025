...
brick_cdf = ecdf(slr_default[:, Symbol(2100)])
targets_q = brick_cdf.(slr_default[:, Symbol(2100)])
targets_ar6 = quantile(ar6_out[:, Symbol(2100)], targets_q)
...
function plot_sos_contours(slr_out, threshold, features, key_params, stepsize, limits, labels, subplot_label, colors, title; f=Figure())
    Random.seed!(2)
    #refit tree using just t_peak, antarctic temp threshold, and climate sensitivity
    slr_labels = ifelse.(threshold .< slr_out, "high", "normal")
